...
def generate_company_variation(company_name: str) -> str:
    ...
    already_with_suffix = False
    for suffix in business_suffixes:
        if suffix in company_name:
            already_with_suffix = True
            break
...
def add_small_variation_to_duplicates(row: List, schema: str) -> List:
    ...
    if schema == 'products':
        stock_index = find_index(PRODUCTS_SCHEMA, "Stock")
        new_row = row.copy()
        new_row[stock_index] = TYPES_TO_GENERATORS['positive_integer']()

        date_index = find_index(PRODUCTS_SCHEMA, "Added Date")
        new_row[date_index] = TYPES_TO_GENERATORS['date']()
        return new_row
    
    if schema == 'organizations':
        new_row = row.copy()

        name_index = find_index(ORGANIZATIONS_SCHEMA, "Name")
        new_row[name_index] = generate_company_variation(row[name_index])

        updated_at_index = find_index(ORGANIZATIONS_SCHEMA, "UpdatedAt")
        new_row[updated_at_index] = TYPES_TO_GENERATORS['datetime']()

        description_index = find_index(ORGANIZATIONS_SCHEMA, "Description")
        new_row[description_index] = TYPES_TO_GENERATORS['long_text']()
        return new_row
